...
function hasClosedCuePrefix(text, cue, { balancedWrapper = false } = {}) {
  ...
  if (
    tail.startsWith(cue)
    && (tail.length === cue.length || !/[\p{L}\p{N}]/u.test(tail[cue.length]))
  ) {
    return true;
  }
  ...
  for (const [opener, closer] of BALANCED_OUTER_WRAPPERS) {
    ...
    if (
      ...
      && (
        ...
        || !/[\p{L}\p{N}]/u.test(tail[wrappedCue.length])
      )
    ) {
      return true;
    }
...
function getBalancedOuterWrapper(chunk, localMatch) {
  ...
  if (
    ...
    return Object.freeze({ opener: prefix, closer: suffix });
  }
  return null;
}
...
function classifySpan(
  sourceText,
  match,
  { completeHostChunk = false, outerWrapper = null } = {}
) {
...
export function analyzePublicReceptionInputText(text) {
  ...
  while ((chunkMatch = chunkMatcher.exec(sourceText)) !== null) {
    ...
    if (normalizedDecodedUrlText(chunk).includes("@")) {
    ...
    if (localMatches.length > 1) {
      ...
      else if (normalizedDecodedUrlText(chunk.slice(coreStart, coreEnd)).toLowerCase().includes(".eth")) {
    ...
    const outerWrapper = coversWholeChunk
      ? null
      : getBalancedOuterWrapper(chunk, localMatch);

    if (!coversWholeChunk && !outerWrapper) {
      ...
      if (normalizedDecodedUrlText(chunk).toLowerCase().includes(".eth")) {
        malformedIdentifier = true;
        continue;
      }
    }

...
const SINGLE_IPV4_AUTHORITY_PATTERN = /^(?:0x[0-9a-f]+|0[0-7]+|\d+):\d*(?:[?#])?$/iu;
// Decimal digits intentionally subsume octal-looking atoms here so the repeated alternatives stay disjoint.
const HYPHEN_PUNCTUATED_IPV4_NUMBER_PATTERN = /^(?=[^:?#]*-)-*(?:0x[0-9a-f]+|\d+)(?:-+(?:0x[0-9a-f]+|\d+))*-*(?::\d*)?(?:[?#])?$/iu;
...
function classifySpan(
  ...
  const numericIpv4AuthorityCandidate = SINGLE_IPV4_AUTHORITY_PATTERN.test(lowerSubject);
  const hyphenPunctuatedIpv4Candidate = HYPHEN_PUNCTUATED_IPV4_NUMBER_PATTERN.test(lowerSubject);

  if (!percentDecoded) {
    if (
      allowScalarExemptions
      && !numericIpv4AuthorityCandidate
      && !hyphenPunctuatedIpv4Candidate
      && (
  ...
  try {
    ...
    if (numericIpv4AuthorityCandidate || hyphenPunctuatedIpv4Candidate || punctuationHostCandidate) {
      return Object.freeze({ kind: "url" });
    }

...
// that ENS classification sees. NFKC and percent decoding happen only after the
// original span is fixed; transformed punctuation can never re-tokenize identity.
...
function classifySpan(sourceText, match) {
  ...
  // A complete source span owns every @ delimiter. WHATWG therefore sees repeated
  ...
  if (subject.includes("@")) {
    try {
      ...
      // Invalid authority-like text remains one indivisible non-ENS span.
    }
  }
...
export function analyzePublicReceptionInputText(text) {
  ...
  while ((match = matcher.exec(sourceText)) !== null) {
    const classification = classifySpan(sourceText, match);

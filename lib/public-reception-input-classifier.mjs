...
function classifySpan(sourceText, match, { completeHostChunk = false } = {}) {
  ...
  if (!percentDecoded) {
    ...
    // A supported ENS identity must own the complete literal caller span. The only
    // punctuation exception is exactly one terminal ASCII sentence dot, and only
    // when removing that dot leaves a complete canonical supported ENS name.
  ...
  try {
    // Host parsing always receives the complete normalized/decoded caller identity.
    // No leading character may be stripped to expose a supported ENS suffix.
    ...
    if (asciiLabels.length >= 2) {
      ...
      if (literalMalformedEnsCandidate) {
        // Literal malformed ENS-like text may remain bounded unsupported only when
        // the caller identity itself starts and ends on non-dot characters. Parsed
        // leading/trailing-dot hosts are URL syntax, not presentation punctuation.
  ...
  // Literal ENS-looking identities that WHATWG itself rejects still fail closed as
  // malformed input rather than falling through to technical keyword routing. This
  // does not grant a host exemption: any complete identity WHATWG accepted already
  // returned `url` above, and percent-decoded identities never enter this lane.

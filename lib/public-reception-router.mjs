  ...
  while ((match = tokenMatcher.exec(hostnameText)) !== null) {
    ...
    try {
      ...
      if (hasCanonicalDnsLabels) {
        // Only a literal, provenance-preserved ENS token may enter the ENS lane.
        // A token synthesized by percent decoding is never allowed to manufacture
        // an ENS exemption because the downstream ENS scanner sees caller text.
        if (
          allowEnsExemption
      ...
      if (asciiLabels.length >= 2) {
        ...
        // are still external destinations. The malformed-ENS lane is available only
        // to literal caller tokens, never to identities created by percent decoding.
...
function hasAuthorityUserinfoSyntax(text) {
  ...
  while ((match = matcher.exec(text)) !== null) {
    try {
      ...
      // Only complete spans accepted by WHATWG as an authority are rejected here.
    }
  }
  return false;
}

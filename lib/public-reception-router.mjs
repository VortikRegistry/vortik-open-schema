...
function extractEnsNames(text) {
  const names = [];
  const matcher = /(?:^|[^A-Za-z0-9-])((?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+eth)(?=$|[^A-Za-z0-9-])/giu;
  let match;
...
export function routePublicReception(input) {
  ...
  const normalizedText = normalizePublicReceptionText(input.text);
  if (!normalizedText) throw new Error("Reception text is empty after normalization");
  const names = extractEnsNames(input.text);

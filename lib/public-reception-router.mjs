...
function createEnsNameMatcher() {
  return /(?:^|[^\p{L}\p{N}_.-])((?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+eth)(?=$|[^\p{L}\p{N}_.-])/giu;
}

function createEnsLikeTokenMatcher() {
  return /(?:^|[^\p{L}\p{N}_.-])([\p{L}\p{N}_.-]*\.eth[\p{L}\p{N}_.-]*)(?=$|[^\p{L}\p{N}_.-])/giu;
}
...
function inspectEnsNames(text) {
  ...
  const ensLikeMatcher = createEnsLikeTokenMatcher();
  let malformedIdentifier = false;
  while ((match = ensLikeMatcher.exec(text)) !== null) {
    const start = match.index + match[0].length - match[1].length;
    if (!validSpanKeys.has(`${start}:${start + match[1].length}`)) {

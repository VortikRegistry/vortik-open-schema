...
function hasSchemelessHostname(text) {
  ...
  while ((match = tokenMatcher.exec(hostnameText)) !== null) {
    ...
    const hasDot = /[.\u3002\uFF0E\uFF61]/u.test(lowerToken);
    ...
    try {
      ...
      if (asciiLabels.length >= 2) {
        ...
        // Only a literal ASCII `.eth` fragment in this same token may enter the closed
        // malformed-ENS lane; Unicode hostname separators do not manufacture that identity.

...
function hasSchemelessHostname(text) {
  ...
  while ((match = tokenMatcher.exec(hostnameText)) !== null) {
    ...
    const isSupportedEnsName = normalizeSupportedEnsName(lowerToken) === lowerToken;
    if (
      isSupportedEnsName
    ...
    try {
      ...
      if (hasCanonicalDnsLabels) {
        // A complete token in the closed ENS alphabet belongs to the ENS validator,
        // even when malformed there. Symbol/IDN attachments that canonicalize into a
        // normal hostname remain caller-controlled destinations and fail at this gate.
        if (
      ...
      if (asciiLabels.length >= 2) {
        // WHATWG accepts host syntax outside canonical DNS labels (for example
        // underscores or edge hyphens). Non-ENS hosts are still external destinations.
        // Tokens already carrying an explicit .eth fragment stay in the closed ENS
        // malformed-token lane and become unsupported without research or retrieval.
        if (lowerToken.includes(".eth")) continue;
        return true;
      }
    } catch {

...
function assertPlainClosedInput(input) {
  ...
  if (/\b(?:https?|ftp):\/\//iu.test(descriptors.text.value) || /\bwww\./iu.test(descriptors.text.value)) {
  ...
  if (
    ...
    || !/^[A-Za-z0-9._:-]{1,128}$/u.test(descriptors.requestId.value)
  ) {
    throw new Error("Reception requestId must be a bounded opaque identifier");
  }
}
...
function createIdentifierTokenMatcher() {
  return /[\p{L}\p{M}\p{N}\p{Pc}\p{Pd}\p{Cf}]+(?:\.[\p{L}\p{M}\p{N}\p{Pc}\p{Pd}\p{Cf}]+)*/giu;
}
...
function inspectEnsNames(text) {
  ...
  while ((match = identifierMatcher.exec(text)) !== null) {
    if (!match[0].toLowerCase().includes(".eth")) continue;
    if (!validSpanKeys.has(`${match.index}:${match.index + match[0].length}`)) {

...
function hasSchemelessHostname(text) {
  ...
  while ((match = tokenMatcher.exec(hostnameText)) !== null) {
    ...
    const hasDot = lowerToken.includes(".");
    if (
    ...
    try {
      ...
      if (isParsedIpv4) {
        if (hasDot || Number(asciiLabels[0]) !== 0) return true;
        continue;
      }
      ...
      if (/\p{L}/u.test(lastLabel) && hasCanonicalDnsLabels) return true;
    } catch {
...
export function hasCallerControlledUrlSyntax(text) {
  ...
  const normalizedText = decodeUrlPercentEscapes(

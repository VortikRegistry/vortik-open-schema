  ...
  while ((match = tokenMatcher.exec(hostnameText)) !== null) {
    ...
    try {
      ...
      if (asciiLabels.length >= 2) {
        ...
        // underscores, edge hyphens or an absolute-host trailing dot). Non-ENS hosts
        // are still external destinations. Only a literal ASCII `.eth` fragment in
        // this same token may enter the closed malformed-ENS lane; Unicode hostname
        // separators do not manufacture that identity.

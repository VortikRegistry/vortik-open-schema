...
function route({ intent, status, routeId, confidence, discoveryGroup, identifier, ensResearch, publicSignal }) {
  return deepFreeze({
...
export function routePublicReception(input) {
  ...
  const normalizedText = normalizePublicReceptionText(maskEnsNamesForIntent(input.text, ensInspection.spans));
  if (!normalizedText && names.length === 0) throw new Error("Reception text is empty after normalization");
  const uniqueIdentifier = names.length === 1 ? names[0] : undefined;

  if (ensInspection.malformedIdentifier) return route({ intent: "unsupported", status: "not_supported", routeId: "public_capability_discovery", confidence: "deterministic", discoveryGroup: "generic" });
  if (includesAny(normalizedText, CANDIDATE_TERMS)) return route({ intent: "candidate_submission", status: uniqueIdentifier ? "routed" : "needs_bounded_identifier", routeId: "public_github_issue_path", confidence: "explicit", discoveryGroup: "contribution", identifier: uniqueIdentifier });
  if (includesAny(normalizedText, EVIDENCE_TERMS)) return route({ intent: "evidence_contribution", status: "routed", routeId: "public_contribution_contract", confidence: "explicit", discoveryGroup: "contribution", identifier: uniqueIdentifier });
  if (includesAny(normalizedText, COMMERCIAL_TERMS)) {
    ...
    return route({
      ...
      publicSignal: {
        ...
        routingReason: recognized ? "explicit_interest_language_with_single_ens_identifier" : "ambiguous_or_missing_ens_identifier",
        confidence: recognized ? "high" : "low",
  ...
  if (names.length === 1) {
    ...
    return route({ intent: "ens_research", status: "completed", routeId: "canonical_local_ens_research", confidence: "deterministic", discoveryGroup: "ens", identifier: names[0], ensResearch });
  }
  if (names.length > 1) return route({ intent: "unsupported", status: "multiple_identifiers_not_supported", routeId: "public_capability_discovery", confidence: "deterministic", discoveryGroup: "generic" });
  if (includesAny(normalizedText, CAPABILITY_TERMS)) return route({ intent: "capability_discovery", status: "completed", routeId: "public_capability_discovery", confidence: "explicit", discoveryGroup: "generic" });
  if (includesAny(normalizedText, REGISTRY_TERMS)) return route({ intent: "registry_lookup", status: "routed", routeId: "allowlisted_public_artifacts", confidence: "explicit", discoveryGroup: "auto" });
  if (includesAny(normalizedText, TECHNICAL_TERMS)) return route({ intent: "technical_context", status: "routed", routeId: "allowlisted_public_artifacts", confidence: "explicit", discoveryGroup: "auto" });
  return route({ intent: "unsupported", status: "not_supported", routeId: "public_capability_discovery", confidence: "deterministic", discoveryGroup: "generic" });
}

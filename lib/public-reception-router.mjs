...
function hasSchemelessHostname(text) {
  const hostnameText = text.replace(/[\u3002\uFF0E\uFF61]/gu, ".");
  const tokenMatcher = /[\p{L}\p{M}\p{N}\p{Pc}\p{Pd}\p{Cf}.]+/gu;
  let match;
  while ((match = tokenMatcher.exec(hostnameText)) !== null) {
    const token = match[0].replace(/^\.+|\.+$/gu, "");
    const lowerToken = token.toLowerCase();
    if (!lowerToken.includes(".") || lowerToken.endsWith(".eth")) continue;

    const labels = lowerToken.split(".");
    const lastLabel = labels.at(-1);
    const isIpv4 = labels.length === 4 && labels.every(
      (label) => /^\d{1,3}$/u.test(label) && Number(label) <= 255
    );
    if (!isIpv4 && !/\p{L}/u.test(lastLabel)) continue;
    ...
    try {
      ...
      if (isIpv4 || hasCanonicalDnsLabels) return true;
    } catch {
...
function createEnsNameMatcher() {
  return /(?:^|[^\p{L}\p{M}\p{N}\p{Pc}\p{Pd}\p{Cf}.])((?:[A-Za-z0-9](?:[A-Za-z0-9-]{0,61}[A-Za-z0-9])?\.)+eth)(?![\p{L}\p{M}\p{N}\p{Pc}\p{Pd}\p{Cf}]|\.[\p{L}\p{M}\p{N}\p{Pc}\p{Pd}\p{Cf}])/giu;
}

function createIdentifierTokenMatcher() {
  return /[\p{L}\p{M}\p{N}\p{Pc}\p{Pd}\p{Cf}.]+/giu;
}

function inspectEnsNames(text) {
  const names = [];
  const matcher = createEnsNameMatcher();
  const spans = [];
  let match;
  while ((match = matcher.exec(text)) !== null) {
    const name = normalizeSupportedEnsName(match[1]);
    if (name === null) continue;
    if (!names.includes(name)) names.push(name);
    const start = match.index + match[0].length - match[1].length;
    spans.push(Object.freeze({ start, end: start + match[1].length }));
  }
  const validSpanKeys = new Set(spans.map(({ start, end }) => `${start}:${end}`));
  const identifierMatcher = createIdentifierTokenMatcher();
  let malformedIdentifier = false;
  while ((match = identifierMatcher.exec(text)) !== null) {
    const identifier = match[0].replace(/\.+$/u, "");
    if (!identifier.toLowerCase().includes(".eth")) continue;
    if (!validSpanKeys.has(`${match.index}:${match.index + identifier.length}`)) {
...
export function routePublicReception(input) {
  ...
  const normalizedInputText = input.text.normalize("NFKC");
  const ensInspection = inspectEnsNames(normalizedInputText);
  const names = ensInspection.names;
  const normalizedText = normalizePublicReceptionText(
    maskEnsNamesForIntent(normalizedInputText, ensInspection.spans)
  );

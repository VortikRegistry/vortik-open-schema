...
function assertNormalizedAsciiEth2ld(name) {
  ...
  if (Buffer.byteLength(label, "utf8") > 63 || !/^[a-z0-9](?:[a-z0-9-]*[a-z0-9])?$/.test(label)) {
...
async function ethCall(provider, to, data, tag) {
  return provider.rpc("eth_call", [{ to, data }, tag]);
}
...
async function deriveProviderLookup(provider, normalizedName, block) {
  ...
  const candidateOwner = parseAddressWord(
    await ethCall(provider, ENS_REGISTRY, callDataBytes32(OWNER_SELECTOR, candidateNode), tag),
    "ENS registry candidate owner"
  );
  const ethRegistrarOwner = parseAddressWord(
    await ethCall(provider, ENS_REGISTRY, callDataBytes32(OWNER_SELECTOR, ethNode), tag),
    "ENS .eth registrar owner"
  );
  const expiry = parseUintWord(
    await ethCall(provider, BASE_REGISTRAR, callDataUint(NAME_EXPIRES_SELECTOR, tokenId), tag),

...
function assertProviderConfig(provider) {
  ...
  if (typeof provider.rpc_url !== "string" || provider.rpc_url.includes("?")) {
    throw new Error("ENS provider rpc_url must not include a query component or empty query delimiter");
  }
  let url;
  try {
  ...
  if (!hostname || hostname.endsWith(".")) throw new Error("ENS provider rpc_url hostname is invalid");
  url.hostname = hostname;
  const endpointAuthority = `${url.hostname}:${url.port || "443"}`;
...
async function readBoundedResponseText(response, timeout) {
  const rawContentLength = response.headers?.get?.("content-length");
  if (rawContentLength !== null && rawContentLength !== undefined) {
    if (!/^\d+$/.test(String(rawContentLength))) throw new Error("ENS RPC response has invalid Content-Length");
    if (Number(rawContentLength) > MAX_RPC_RESPONSE_BYTES) throw new Error("ENS RPC response exceeds verifier size limit");
  }
  if (!response.body || typeof response.body.getReader !== "function") {
    throw new Error("ENS RPC response body must be a readable stream");
  }
  ...
  try {
    while (true) {
      ...
      if (!chunk || typeof chunk !== "object") throw new Error("ENS RPC response stream returned an invalid chunk");
      if (chunk.done) break;
      if (!(chunk.value instanceof Uint8Array)) throw new Error("ENS RPC response stream returned non-byte data");
      byteCount += chunk.value.byteLength;
      if (byteCount > MAX_RPC_RESPONSE_BYTES) {
        void reader.cancel().catch(() => {});
        throw new Error("ENS RPC response exceeds verifier size limit");
      }
...
function bindProvider(provider, requestTimeoutMs) {
  ...
  return Object.freeze({
    ...
    endpoint_authority: trusted.endpoint_authority,
    async rpc(method, params) {
      ...
      try {
        ...
        const text = await readBoundedResponseText(response, timeout);
        let json;
        try { json = JSON.parse(text); } catch { throw new Error(`ENS RPC ${trusted.provider_id} returned invalid JSON`); }
        if (json?.jsonrpc !== "2.0" || json?.id !== currentRequestId) throw new Error(`ENS RPC ${trusted.provider_id} returned mismatched JSON-RPC metadata`);
...
export function createEnsMainnetVerifierWithTrustedProviders({
  ...
  if (new Set(boundProviders.map((provider) => provider.endpoint_authority)).size !== 2) throw new Error("ENS verifier requires two distinct provider network authorities");

...
const MAX_GITHUB_REQUEST_TIMEOUT_MS = 30_000;
...
function assertRequestTimeoutMs(value) {
  if (!Number.isSafeInteger(value) || value < 1 || value > MAX_GITHUB_REQUEST_TIMEOUT_MS) {
    throw new TypeError(`primary-source verifier request timeout must be an integer from 1 to ${MAX_GITHUB_REQUEST_TIMEOUT_MS} ms`);
  }
}

function bindTrustedFetchJson(fetchImpl, requestTimeoutMs) {
  if (typeof fetchImpl !== "function") {
    throw new TypeError("primary-source verifier requires a trusted fetch transport at construction");
  }
  assertRequestTimeoutMs(requestTimeoutMs);
  const trustedFetch = fetchImpl;

  return async function fetchJsonExact(url) {
    const controller = new AbortController();
    let timeoutId;
    const timeout = new Promise((_, reject) => {
      timeoutId = setTimeout(() => {
        controller.abort();
        reject(new Error(`trusted GitHub retrieval timed out after ${requestTimeoutMs} ms`));
      }, requestTimeoutMs);
    });
    ...
    try {
      ...
      return await Promise.race([Promise.resolve().then(() => response.json()), timeout]);
    } finally {
...
async function verifyPrimarySourceWithTransport({ claim, selector }, fetchJsonExact) {
...
export function createPrimarySourceVerifierWithTrustedTransport({
  fetchImpl,
  requestTimeoutMs = DEFAULT_GITHUB_REQUEST_TIMEOUT_MS
}) {
  const fetchJsonExact = bindTrustedFetchJson(fetchImpl, requestTimeoutMs);
  return Object.freeze({
    verify(input) {
      return verifyPrimarySourceWithTransport(input, fetchJsonExact);
    }
  });
}

const DEFAULT_PRIMARY_SOURCE_VERIFIER = MODULE_FETCH
  ? createPrimarySourceVerifierWithTrustedTransport({ fetchImpl: MODULE_FETCH })
  : null;

export async function verifyPrimarySourceFromGitHub(input) {
  if (!DEFAULT_PRIMARY_SOURCE_VERIFIER) {
    throw new TypeError("primary-source verifier requires runtime fetch during trusted module initialization");
  }
  return DEFAULT_PRIMARY_SOURCE_VERIFIER.verify(input);
}

export { DEFAULT_GITHUB_REQUEST_TIMEOUT_MS, MAX_SOURCE_BYTES };

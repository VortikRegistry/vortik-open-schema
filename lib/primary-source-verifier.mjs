...
function resolveAllowedRepository(claim, selector, policy) {
  assertPlainObject(claim, "verification claim");
  assertPlainObject(policy, "trusted primary-source policy");
  if (policy.provider !== "github" || policy.policy_id !== "vortik-primary-source-github-v1" || !Array.isArray(policy.repositories)) {
    throw new Error("unsupported trusted primary-source policy");
  }
  ...
  const matches = policy.repositories.filter((entry) => entry.repository_full_name === selector.repository_full_name);
  ...
  if (!Number.isSafeInteger(entry.repository_id) || entry.repository_id <= 0) {
    throw new Error("trusted primary-source policy has invalid repository identity");
  }
...
function decodeGitHubContent(file) {
  ...
  if (bytes.length > MAX_SOURCE_BYTES) throw new Error("primary-source artifact exceeds verifier byte limit");
  return bytes;
}
...
export async function verifyPrimarySourceFromGitHub({
  claim,
  selector,
  fetchImpl = globalThis.fetch,
  policy = DEFAULT_PRIMARY_SOURCE_GITHUB_POLICY
}) {
  if (typeof fetchImpl !== "function") throw new TypeError("primary-source verifier requires fetch");
  assertSourceSelector(selector);
  const { entry, authorityClass } = resolveAllowedRepository(claim, selector, policy);

  const repository = await fetchJsonExact(fetchImpl, githubRepositoryUrl(selector.repository_full_name));
  ...
  const file = await fetchJsonExact(fetchImpl, githubPathUrl(selector.repository_full_name, selector.path, selector.commit_sha));
  if (file?.path !== selector.path || typeof file?.sha !== "string" || !/^[0-9a-f]{40}$/.test(file.sha)) {

...
export async function verifyPrimarySourceFromGitHub({ claim, selector }) {
  ...
  const { entry, authorityClass } = resolveAllowedRepository(claim, selector);

  const repository = await fetchJsonExact(githubRepositoryUrl(selector.repository_full_name));
  ...
  const file = await fetchJsonExact(githubPathUrl(selector.repository_full_name, selector.path, selector.commit_sha));

...
function writeA2AError(response, statusCode, status, message, reason) {
  const error = {
    code: statusCode,
    status,
    message
  ...
  writeA2AJson(response, statusCode, { error });
}
  ...
  return async function handlePublicA2ARequest(request, response) {
    ...
    if (url.pathname === "/a2a/v1/message:send") {
      if (request.method !== "POST" || url.search || url.hash) {
        writeA2AError(response, request.method === "POST" ? 400 : 405, request.method === "POST" ? "INVALID_ARGUMENT" : "METHOD_NOT_ALLOWED", "SendMessage accepts only plain POST requests");
        return;
      }
      if (!budget.consume()) {
    ...
    if (url.pathname === "/a2a/v1/tasks") {
      if (request.method !== "GET") {
        writeA2AError(response, 405, "METHOD_NOT_ALLOWED", "Task listing accepts only GET");
        return;
      }
      if (url.hash) {
    ...
    if (taskCancelMatch) {
      if (request.method !== "POST") {
        writeA2AError(response, 405, "METHOD_NOT_ALLOWED", "Task cancel accepts only POST");
        return;
      }
      try {
    ...
    if (taskGetMatch) {
      if (request.method !== "GET") {
        writeA2AError(response, 405, "METHOD_NOT_ALLOWED", "Task retrieval accepts only GET");
        return;
      }
      try {

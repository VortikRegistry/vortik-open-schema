...
function writeA2AStatus(response, httpStatus, rpcCode, message, reason) {
  const status = { code: rpcCode, message };
  if (reason) {
    status.details = [{
  ...
  writeA2AJson(response, httpStatus, status);
}
...
function writeMappedError(response, error) {
  switch (error?.a2aReason) {
    case "TASK_NOT_FOUND":
      writeA2AStatus(response, 404, RPC_CODE.NOT_FOUND, "The specified task ID does not exist or is not accessible", "TASK_NOT_FOUND");
      return;
    case "PUSH_NOTIFICATION_NOT_SUPPORTED":
      writeA2AStatus(response, 400, RPC_CODE.FAILED_PRECONDITION, "Push notifications are not supported by this agent", "PUSH_NOTIFICATION_NOT_SUPPORTED");
      return;
    case "CONTENT_TYPE_NOT_SUPPORTED":
      writeA2AStatus(response, 400, RPC_CODE.INVALID_ARGUMENT, "The requested content type is not supported by this agent", "CONTENT_TYPE_NOT_SUPPORTED");
      return;
    case "VERSION_NOT_SUPPORTED":
      writeA2AStatus(response, 400, RPC_CODE.FAILED_PRECONDITION, `A2A protocol version ${A2A_PROTOCOL_VERSION} is required`, "VERSION_NOT_SUPPORTED");
      return;
    default:
      break;
  }
  ...
  if (error?.requestCode === "PAYLOAD_TOO_LARGE") {
    writeA2AStatus(response, 413, RPC_CODE.RESOURCE_EXHAUSTED, "Request body exceeds the beacon limit");
    return;
  }
  if (error?.requestCode === "INVALID_JSON") {
    writeA2AStatus(response, 400, RPC_CODE.INVALID_ARGUMENT, "Request body is not valid JSON");
    return;
  }
  writeA2AStatus(response, 400, RPC_CODE.INVALID_ARGUMENT, "Request does not match the bounded discovery contract");
}
...
function writeUnsupportedOperation(response) {
  writeA2AStatus(response, 400, RPC_CODE.FAILED_PRECONDITION, "This operation is not supported by the read-only discovery beacon", "UNSUPPORTED_OPERATION");
}
  ...
  return async function handlePublicA2ARequest(request, response) {
    ...
    if (url.pathname === "/a2a/v1/message:send") {
      if (request.method !== "POST" || url.search || url.hash) {
        writeA2AStatus(response, request.method === "POST" ? 400 : 405, RPC_CODE.INVALID_ARGUMENT, "SendMessage accepts only plain POST requests");
        return;
      }
      if (!budget.consume()) {
        writeA2AStatus(response, 429, RPC_CODE.RESOURCE_EXHAUSTED, "Beacon request budget is exhausted");
        return;
      }
      try {
    ...
    if (url.pathname === "/a2a/v1/extendedAgentCard") {
      writeA2AStatus(response, 400, RPC_CODE.FAILED_PRECONDITION, "An extended Agent Card is not configured", "EXTENDED_AGENT_CARD_NOT_CONFIGURED");
      return;
    }

    if (url.pathname === "/a2a/v1/tasks") {
      if (request.method !== "GET") {
        writeA2AStatus(response, 405, RPC_CODE.INVALID_ARGUMENT, "Task listing accepts only GET");
        return;
      }
      if (url.hash) {
        writeA2AStatus(response, 400, RPC_CODE.INVALID_ARGUMENT, "Task list request is invalid");
        return;
      }
      try {
        ...
        writeA2AStatus(response, 400, RPC_CODE.INVALID_ARGUMENT, "Task list query is invalid");
      }
      return;
    }
    ...
    if (taskCancelMatch) {
      if (request.method !== "POST") {
        writeA2AStatus(response, 405, RPC_CODE.INVALID_ARGUMENT, "Task cancel accepts only POST");
        return;
      }
      try {
        ...
        writeA2AStatus(response, 400, RPC_CODE.INVALID_ARGUMENT, "Task cancel query is invalid");
        return;
      }
      writeA2AStatus(response, 404, RPC_CODE.NOT_FOUND, "The specified task ID does not exist or is not accessible", "TASK_NOT_FOUND");
      return;
    ...
    if (taskGetMatch) {
      if (request.method !== "GET") {
        writeA2AStatus(response, 405, RPC_CODE.INVALID_ARGUMENT, "Task retrieval accepts only GET");
        return;
      }
      try {
        ...
        writeA2AStatus(response, 400, RPC_CODE.INVALID_ARGUMENT, "Task retrieval query is invalid");
        return;
      }
      writeA2AStatus(response, 404, RPC_CODE.NOT_FOUND, "The specified task ID does not exist or is not accessible", "TASK_NOT_FOUND");
      return;
    ...
    writeA2AStatus(response, 404, RPC_CODE.NOT_FOUND, "A2A route not found");
  };
}

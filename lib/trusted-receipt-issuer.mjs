...
export function createTrustedReceiptIssuerCore({
  ...
  return Object.freeze({
    issuePrimarySourceReceipt(input) {
      ...
      return issue({
        ...
        derivePayload: (claim) => primary.verifier.verify({ claim, selector: input.selector })
      });
    },
    issueEnsMainnetReceipt(input) {
      ...
      return issue({
        ...
        intent: input.admissionIntent,
        verifierBinding: ens,
        derivePayload: (claim) => ens.verifier.verify({ normalizedCandidateName: claim.normalized_candidate_name })
      });
    }
  });
}

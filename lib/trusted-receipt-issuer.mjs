...
const KEY_POLICY_SCHEMA_URL = new URL("../schemas/verification/vortik-verification-key-policy/1.0.0/schema.json", import.meta.url);
...
const validateKeyPolicyContract = contractAjv.compile(JSON.parse(readFileSync(KEY_POLICY_SCHEMA_URL, "utf8")));
...
function bindKeyPolicy(keyPolicy, trustedPolicyIdentity) {
  ...
  assertContract(validateKeyPolicyContract, policy, "verification key policy");

...
export function assertReceiptEvidenceSemantics(receipt, claim) {
  if (receipt.receipt_type === "primary_source") {
    if (!claim || typeof claim !== "object" || Array.isArray(claim)) {
      throw new TypeError("primary-source receipt evidence requires the bound verification claim");
    }
    if (receipt.subject.claim_digest !== sha256CanonicalDigest(claim)) {
      throw new Error("primary-source receipt subject is not bound to the supplied verification claim");
    }
    if (receipt.payload.authority_class !== claim?.technical_claim?.source_authority_class) {
      throw new Error("primary-source authority class must match the verification claim");
    }
...
function assertKeyPolicySemantics(keyPolicy) {
  ...
  for (const key of keyPolicy.authorized_keys) {
    ...
    const publicKeyBytes = Buffer.from(key.public_key_spki_der_base64, "base64");
    const publicKeyIdentity = publicKeyBytes.toString("hex");
    if (publicKeys.has(publicKeyIdentity)) throw new Error("key policy aliases one public key under multiple identities");
    if (key.not_before > key.not_after) throw new Error("key policy authorization window is inverted");
    keyIds.add(key.key_id);
    publicKeys.add(publicKeyIdentity);
  }
}

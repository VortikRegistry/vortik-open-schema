...
export const SANITIZED_SIGNAL_LOG_SCHEMA = "vortik_sanitized_commercial_signal_log/1.0.0";
...
function readDataDescriptors(value, label) {
  ...
  try {
    ...
    throw new TypeError(`${label} could not be safely inspected`);
  }
...
export function createSanitizedCommercialSignalLogEvent(signal) {
  const validatedSignal = assertPrivateSignalContract(signal);
  return Object.freeze({
    severity: "WARNING",
    schema: SANITIZED_SIGNAL_LOG_SCHEMA,
    event_type: "sanitized_commercial_signal",

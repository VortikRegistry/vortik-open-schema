import canonicalCoordinationSurfaces from "../maps/coordination-surfaces.json" with { type: "json" };

import { immutableJsonSnapshot } from "./immutable-json-snapshot.mjs";

const canonicalCoordinationSurfacesSnapshot = immutableJsonSnapshot(
  canonicalCoordinationSurfaces
);

export const ENS_RESEARCH_REQUEST_SCHEMA_ID =
  "https://raw.githubusercontent.com/VortikRegistry/vortik-open-schema/main/schemas/queries/vortik-ens-research-request/1.0.0/schema.json";
export const ENS_RESEARCH_RESPONSE_SCHEMA_ID =
  "https://raw.githubusercontent.com/VortikRegistry/vortik-open-schema/main/schemas/queries/vortik-ens-research-response/1.0.0/schema.json";

const REQUEST_KEYS = new Set([
  "$schema",
  "request",
  "request_version",
  "request_id",
  "query"
]);
const QUERY_KEYS = new Set(["name"]);
const EVIDENCE_ARTIFACT_KEYS = new Set(["coordinationSurfaces"]);
const COORDINATION_SURFACES_KEYS = new Set([
  "model",
  "version",
  "type",
  "description",
  "last_updated",
  "surfaces",
  "ambiguous_surfaces",
  "coverage",
  "notes"
]);
const SURFACE_KEYS = new Set([
  "id",
  "name",
  "anchors",
  "category",
  "description"
]);
const AMBIGUOUS_SURFACE_KEYS = new Set([
  "id",
  "name",
  "anchors",
  "description"
]);
const COVERAGE_KEYS = new Set([
  "registry_anchors_mapped",
  "coverage_status"
]);
const ASCII_NORMALIZED_ENS =
  /^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+eth$/u;
const CLASSIFICATIONS = new Set([
  "core",
  "repairable",
  "premature",
  "external",
  "deprecated"
]);
const ANCHOR_TYPES = new Set([
  "primitive",
  "constraint",
  "external_actor",
  "external_mechanism",
  "coordination_surface",
  "misaligned_abstraction"
]);

function codePoints(value) {
  return Array.from(value);
}

function assertObject(value, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    throw new Error(`${label} must be an object`);
  }
}

function assertExactKeys(value, allowedKeys, label) {
  assertObject(value, label);
  const unexpected = Object.keys(value).filter((key) => !allowedKeys.has(key));
  const missing = [...allowedKeys].filter(
    (key) => !Object.prototype.hasOwnProperty.call(value, key)
  );

  if (unexpected.length > 0 || missing.length > 0) {
    throw new Error(`${label} fields do not match the closed contract`);
  }
}

function assertAllowedKeys(value, allowedKeys, label) {
  assertObject(value, label);
  const unexpected = Object.keys(value).filter((key) => !allowedKeys.has(key));
  if (unexpected.length > 0) {
    throw new Error(`${label} contains unsupported fields`);
  }
}

function assertBoundedString(value, label, maximumLength) {
  if (
    typeof value !== "string"
    || value.length === 0
    || codePoints(value).length > maximumLength
  ) {
    throw new Error(`${label} must be a non-empty bounded string`);
  }
}

function exactJsonValue(value, canonicalValue) {
  if (value === canonicalValue) return true;

  if (Array.isArray(canonicalValue)) {
    return (
      Array.isArray(value)
      && value.length === canonicalValue.length
      && canonicalValue.every((entry, index) =>
        exactJsonValue(value[index], entry)
      )
    );
  }

  if (
    !canonicalValue
    || typeof canonicalValue !== "object"
    || !value
    || typeof value !== "object"
    || Array.isArray(value)
  ) {
    return false;
  }

  const canonicalKeys = Object.keys(canonicalValue);
  const valueKeys = Object.keys(value);
  return (
    valueKeys.length === canonicalKeys.length
    && canonicalKeys.every(
      (key) =>
        Object.prototype.hasOwnProperty.call(value, key)
        && exactJsonValue(value[key], canonicalValue[key])
    )
  );
}

function assertRequest(request) {
  assertExactKeys(request, REQUEST_KEYS, "ENS research request");
  if (
    request.$schema !== ENS_RESEARCH_REQUEST_SCHEMA_ID
    || request.request !== "vortik-ens-research-request"
    || request.request_version !== "1.0.0"
  ) {
    throw new Error("Unsupported ENS research request contract");
  }
  if (
    typeof request.request_id !== "string"
    || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(request.request_id)
  ) {
    throw new Error("ENS research request_id is invalid");
  }

  assertExactKeys(request.query, QUERY_KEYS, "ENS research query");
  if (
    typeof request.query.name !== "string"
    || codePoints(request.query.name).length > 4096
  ) {
    throw new Error("ENS research query name is outside the request contract");
  }
}

function assertRegistryAnchor(anchor, index) {
  const label = `Registry anchor ${index}`;
  assertObject(anchor, label);

  if (typeof anchor.id !== "string" || !/^[a-z0-9-]+$/u.test(anchor.id)) {
    throw new Error(`${label} id is invalid`);
  }
  if (
    typeof anchor.ens !== "string"
    || normalizeSupportedEnsName(anchor.ens) !== anchor.ens
  ) {
    throw new Error(`${label} ENS name is outside the supported normalized subset`);
  }
  assertBoundedString(anchor.canonical_term, `${label} canonical_term`, 300);
  if (!CLASSIFICATIONS.has(anchor.classification)) {
    throw new Error(`${label} classification is invalid`);
  }
  assertBoundedString(anchor.status, `${label} status`, 100);
  if (!ANCHOR_TYPES.has(anchor.type)) {
    throw new Error(`${label} type is invalid`);
  }
  if (
    typeof anchor.schema !== "string"
    || !/^schemas\/.+\/schema\.json$/u.test(anchor.schema)
  ) {
    throw new Error(`${label} schema path is invalid`);
  }
  if (
    typeof anchor.anchor_doc !== "string"
    || !/^anchors\/.+\.md$/u.test(anchor.anchor_doc)
  ) {
    throw new Error(`${label} document path is invalid`);
  }
}

function assertRegistry(registry) {
  assertObject(registry, "Vortik registry");
  if (
    registry.registry !== "vortik-semantic-registry"
    || registry.source_of_truth !== "schemas"
    || typeof registry.version !== "string"
    || !/^[0-9]+\.[0-9]+\.[0-9]+$/u.test(registry.version)
    || typeof registry.last_updated !== "string"
    || !/^\d{4}-\d{2}-\d{2}$/u.test(registry.last_updated)
    || !Array.isArray(registry.anchors)
  ) {
    throw new Error("Vortik registry metadata is invalid");
  }

  if (registry.anchors.length === 0) {
    throw new Error("Vortik registry must contain at least one anchor");
  }

  registry.anchors.forEach(assertRegistryAnchor);
  const ids = registry.anchors.map((anchor) => anchor.id);
  const names = registry.anchors.map((anchor) => anchor.ens);
  if (new Set(ids).size !== ids.length || new Set(names).size !== names.length) {
    throw new Error("Vortik registry anchors must have unique ids and ENS names");
  }
}

function assertSurfaceEntry(surface, index, registryIds, ambiguous) {
  const label = ambiguous
    ? `Ambiguous coordination surface ${index}`
    : `Coordination surface ${index}`;
  assertExactKeys(
    surface,
    ambiguous ? AMBIGUOUS_SURFACE_KEYS : SURFACE_KEYS,
    label
  );
  if (
    typeof surface.id !== "string"
    || normalizeSupportedEnsName(`${surface.id}.eth`) !== `${surface.id}.eth`
  ) {
    throw new Error(`${label} id is invalid`);
  }
  assertBoundedString(surface.name, `${label} name`, 300);
  assertBoundedString(surface.description, `${label} description`, 1000);
  if (!ambiguous) {
    assertBoundedString(surface.category, `${label} category`, 100);
  }
  if (
    !Array.isArray(surface.anchors)
    || surface.anchors.length === 0
    || surface.anchors.length > 20
    || new Set(surface.anchors).size !== surface.anchors.length
    || surface.anchors.some(
      (anchorId) =>
        typeof anchorId !== "string" || !registryIds.has(anchorId)
    )
  ) {
    throw new Error(`${label} anchors are invalid`);
  }
}

function assertCoordinationSurfaces(value, registry) {
  assertExactKeys(
    value,
    COORDINATION_SURFACES_KEYS,
    "Coordination surfaces artifact"
  );
  if (
    value.model !== "vortik-coordination-surfaces"
    || typeof value.version !== "string"
    || !/^[0-9]+\.[0-9]+\.[0-9]+$/u.test(value.version)
    || value.type !== "semantic-layer"
    || typeof value.description !== "string"
    || codePoints(value.description).length === 0
    || codePoints(value.description).length > 2000
    || typeof value.last_updated !== "string"
    || !/^\d{4}-\d{2}-\d{2}$/u.test(value.last_updated)
    || !Array.isArray(value.surfaces)
    || value.surfaces.length === 0
    || value.surfaces.length > 100
    || !Array.isArray(value.ambiguous_surfaces)
    || value.ambiguous_surfaces.length > 100
    || !Array.isArray(value.notes)
    || value.notes.length > 50
    || value.notes.some(
      (note) =>
        typeof note !== "string"
        || codePoints(note).length === 0
        || codePoints(note).length > 1000
    )
  ) {
    throw new Error("Coordination surfaces artifact metadata is invalid");
  }

  if (!exactJsonValue(value, canonicalCoordinationSurfacesSnapshot)) {
    throw new Error(
      "Coordination surfaces artifact does not match the canonical repository map"
    );
  }

  const registryIds = new Set(registry.anchors.map((anchor) => anchor.id));
  value.surfaces.forEach((surface, index) =>
    assertSurfaceEntry(surface, index, registryIds, false)
  );
  value.ambiguous_surfaces.forEach((surface, index) =>
    assertSurfaceEntry(surface, index, registryIds, true)
  );

  const ids = [
    ...value.surfaces.map((surface) => surface.id),
    ...value.ambiguous_surfaces.map((surface) => surface.id)
  ];
  if (new Set(ids).size !== ids.length) {
    throw new Error("Coordination surface ids must be unique");
  }

  assertExactKeys(
    value.coverage,
    COVERAGE_KEYS,
    "Coordination surfaces coverage"
  );
  const mappedIds = [
    ...value.surfaces.flatMap((surface) => surface.anchors),
    ...value.ambiguous_surfaces.flatMap((surface) => surface.anchors)
  ];
  if (
    !Array.isArray(value.coverage.registry_anchors_mapped)
    || value.coverage.registry_anchors_mapped.length > 100
    || new Set(value.coverage.registry_anchors_mapped).size
      !== value.coverage.registry_anchors_mapped.length
    || value.coverage.registry_anchors_mapped.some(
      (anchorId) =>
        typeof anchorId !== "string" || !registryIds.has(anchorId)
    )
    || value.coverage.coverage_status
      !== `complete_for_registry_v${registry.version}`
    || registryIds.size !== new Set(mappedIds).size
    || [...registryIds].some((anchorId) => !mappedIds.includes(anchorId))
    || registryIds.size !== value.coverage.registry_anchors_mapped.length
    || [...registryIds].some(
      (anchorId) =>
        !value.coverage.registry_anchors_mapped.includes(anchorId)
    )
  ) {
    throw new Error(
      "Coordination surfaces coverage does not match the registry"
    );
  }
}

export function normalizeSupportedEnsName(value) {
  if (typeof value !== "string" || !/^[\x00-\x7F]*$/u.test(value)) return null;

  const candidate = value.normalize("NFC").toLowerCase();
  if (codePoints(candidate).length > 255 || !ASCII_NORMALIZED_ENS.test(candidate)) {
    return null;
  }

  const labels = candidate.split(".").slice(0, -1);
  const hasReservedExtension = labels.some(
    (label) => label.length >= 4 && label[2] === "-" && label[3] === "-"
  );
  return hasReservedExtension ? null : candidate;
}

function responseBase(request, rawName, normalizedName) {
  const rawPoints = codePoints(rawName);
  return {
    $schema: ENS_RESEARCH_RESPONSE_SCHEMA_ID,
    response: "vortik-ens-research-response",
    response_version: "1.0.0",
    request_id: request.request_id,
    query: {
      submitted_name: rawPoints.slice(0, 255).join(""),
      submitted_name_truncated: rawPoints.length > 255,
      normalized_name: normalizedName
    },
    authority: {
      registry_scope: "independent semantic registry",
      protocol_authority: false,
      ens_authority: false,
      ownership_inference: false
    }
  };
}

function invalidResponse(request, rawName) {
  return {
    ...responseBase(request, rawName, null),
    result: {
      state: "invalid_input",
      registry_entry: null,
      related_terms: [],
      evidence: [],
      limitations: [],
      errors: [
        {
          code: "invalid_input",
          message:
            "The submitted value is outside the currently supported fail-closed ASCII ENS subset."
        }
      ]
    }
  };
}

function trackedResponse(request, rawName, normalizedName, anchor, anchorIndex) {
  return {
    ...responseBase(request, rawName, normalizedName),
    result: {
      state: "tracked_anchor",
      registry_entry: {
        id: anchor.id,
        ens: anchor.ens,
        canonical_term: anchor.canonical_term,
        classification: anchor.classification,
        status: anchor.status,
        type: anchor.type,
        schema_path: anchor.schema,
        anchor_doc: anchor.anchor_doc
      },
      related_terms: [],
      evidence: [
        {
          kind: "registry",
          reference: `registry.json#/anchors/${anchorIndex}`,
          claim: "The normalized name exactly matches a validated registry anchor."
        }
      ],
      limitations: [
        "Registry inclusion is not protocol authority or endorsement."
      ],
      errors: []
    }
  };
}

function untrackedResponse(request, rawName, normalizedName) {
  return {
    ...responseBase(request, rawName, normalizedName),
    result: {
      state: "untracked",
      registry_entry: null,
      related_terms: [],
      evidence: [],
      limitations: [
        "Absence from Vortik means only that the current curated registry has no source-grounded assessment for this name."
      ],
      errors: []
    }
  };
}

function indeterminateResponse(request, rawName, normalizedName, code, message) {
  return {
    ...responseBase(request, rawName, normalizedName),
    result: {
      state: "indeterminate",
      registry_entry: null,
      related_terms: [],
      evidence: [],
      limitations: [
        "No semantic conclusion is produced when required curated relation evidence is unavailable or invalid."
      ],
      errors: [{ code, message }]
    }
  };
}

function findSurfaceMatch(normalizedName, coordinationSurfaces) {
  const labels = normalizedName.split(".");
  if (labels.length !== 2) return null;

  const queryLabel = labels[0];
  const regularIndex = coordinationSurfaces.surfaces.findIndex(
    (surface) => surface.id === queryLabel
  );
  if (regularIndex !== -1) {
    return {
      surface: coordinationSurfaces.surfaces[regularIndex],
      collection: "surfaces",
      index: regularIndex,
      ambiguous: false
    };
  }

  const ambiguousIndex = coordinationSurfaces.ambiguous_surfaces.findIndex(
    (surface) => surface.id === queryLabel
  );
  return ambiguousIndex === -1
    ? null
    : {
        surface: coordinationSurfaces.ambiguous_surfaces[ambiguousIndex],
        collection: "ambiguous_surfaces",
        index: ambiguousIndex,
        ambiguous: true
      };
}

function relatedResponse(request, rawName, normalizedName, match, registry) {
  const evidence = [
    {
      kind: "vortik_interpretation",
      reference:
        `maps/coordination-surfaces.json#/${match.collection}/${match.index}`,
      claim:
        "The normalized single-label query exactly matches this curated Vortik surface identifier."
    }
  ];
  const relatedTerms = match.surface.anchors.map((anchorId) => {
    const anchorIndex = registry.anchors.findIndex(
      (anchor) => anchor.id === anchorId
    );
    const anchor = registry.anchors[anchorIndex];
    evidence.push({
      kind: "registry",
      reference: `registry.json#/anchors/${anchorIndex}`,
      claim:
        "This validated registry anchor is mapped to the matched curated Vortik surface."
    });
    return {
      term: anchor.canonical_term,
      relationship: match.ambiguous
        ? "ambiguous_curated_surface"
        : "same_curated_surface",
      evidence_refs: [0, evidence.length - 1]
    };
  });

  const limitations = [
    "Relationship confidence is bounded to exact equality between the queried single label and a curated Vortik surface id; no fuzzy matching or external content was used.",
    "A semantic relationship does not imply ownership, authority, endorsement, or adoption."
  ];
  if (match.ambiguous) {
    limitations.push(
      "The matched Vortik surface is explicitly marked ambiguous and non-canonical."
    );
  }

  return {
    ...responseBase(request, rawName, normalizedName),
    result: {
      state: "related_terminology",
      registry_entry: null,
      related_terms: relatedTerms,
      evidence,
      limitations,
      errors: []
    }
  };
}

export function evaluateEnsResearch(request, registry, evidenceArtifacts = {}) {
  assertRequest(request);
  assertRegistry(registry);
  assertAllowedKeys(
    evidenceArtifacts,
    EVIDENCE_ARTIFACT_KEYS,
    "ENS research evidence artifacts"
  );

  const rawName = request.query.name;
  const normalizedName = normalizeSupportedEnsName(rawName);
  if (normalizedName === null) {
    return invalidResponse(request, rawName);
  }

  const anchorIndex = registry.anchors.findIndex(
    (anchor) => anchor.ens === normalizedName
  );
  if (anchorIndex !== -1) {
    return trackedResponse(
      request,
      rawName,
      normalizedName,
      registry.anchors[anchorIndex],
      anchorIndex
    );
  }

  const relationEvidenceRequested = Object.prototype.hasOwnProperty.call(
    evidenceArtifacts,
    "coordinationSurfaces"
  );
  if (!relationEvidenceRequested) {
    return untrackedResponse(request, rawName, normalizedName);
  }

  if (!evidenceArtifacts.coordinationSurfaces) {
    return indeterminateResponse(
      request,
      rawName,
      normalizedName,
      "curated_evidence_unavailable",
      "The curated coordination-surfaces artifact is required to assess related terminology."
    );
  }

  try {
    assertCoordinationSurfaces(
      evidenceArtifacts.coordinationSurfaces,
      registry
    );
  } catch {
    return indeterminateResponse(
      request,
      rawName,
      normalizedName,
      "curated_evidence_invalid",
      "The curated coordination-surfaces artifact failed closed validation."
    );
  }

  const surfaceMatch = findSurfaceMatch(
    normalizedName,
    evidenceArtifacts.coordinationSurfaces
  );
  return surfaceMatch
    ? relatedResponse(
        request,
        rawName,
        normalizedName,
        surfaceMatch,
        registry
      )
    : untrackedResponse(request, rawName, normalizedName);
}

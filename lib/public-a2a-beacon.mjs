...
const HUMAN_CONTACT_LINK = Object.freeze({
  rel: "human-contact",
  href: "https://x.com/VortikRegistry"
});
...
function assertGeneratedId(value, label) {
  if (typeof value !== "string" || !/^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/u.test(value)) {
    throw new Error(`${label} must match the ENS-compatible generated identifier contract`);
  }
}
...
function assertDiscoveryText(text) {
  ...
  if (hasCallerControlledUrlSyntax(text)) {
...
function responseData(group, reception) {
  const commercial = reception.intent === "commercial_interest";
  return Object.freeze({
    capabilityId: group.capabilityId,
    title: group.title,
    summary: commercial ? `${group.summary} ${HUMAN_CONTACT_NOTICE}` : group.summary,
    tags: [...group.tags],
    links: [
      ...group.links.map((link) => ({ ...link })),
      ...(commercial ? [{ ...HUMAN_CONTACT_LINK }] : [])
    ],
    authority: { ...AUTHORITY },
...
function responseText(group, reception) {
  ...
  if (reception.intent === "commercial_interest") {
    return [
      ...
      `Human contact (buyer-initiated): ${HUMAN_CONTACT_LINK.href}`,
...
export function buildPublicA2AAgentCard({ publicBaseUrl }) {
  ...
  return Object.freeze({
    ...
    skills: Object.freeze([
      ...
      Object.freeze({ id: "ethereum-inclusion-list-semantics", name: "Ethereum inclusion-list semantics", description: "Locate Vortik public artifacts for inclusion-list and FOCIL terminology.", tags: Object.freeze(["ethereum", "inclusion-list", "semantics"]) }),
...
export function createPublicA2ABeacon({ publicBaseUrl, idFactory = randomUUID } = {}) {
  ...
  return Object.freeze({
    ...
    sendMessage(request) {
      ...
      if (validated.contextId === undefined) {
        assertGeneratedId(contextId, "generated contextId");
      } else {
        assertBoundedId(contextId, "contextId", { required: true });
      }
      assertGeneratedId(messageId, "generated messageId");

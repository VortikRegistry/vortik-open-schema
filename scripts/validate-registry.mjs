...
function validateAnchorEntry(anchor) {
  ...
  assert(fs.existsSync(schemaPath), `Schema file not found for "${anchor.id}": ${anchor.schema}`);
  assert(fs.existsSync(docPath), `Anchor doc not found for "${anchor.id}": ${anchor.anchor_doc}`);
